...
void dagmcinit_(char* cfile, int* clen,   // geom
  ...
  *moab_version = DAG->interface_revision();
  ...
  // This is bad because MOAB may divide by zero and expect to continue
  // executing. See MOAB mailing list discussion on April 28 2010. As a
  // workaround, put a hold exceptions when Cubit is present.

  fenv_t old_fenv;
  if (feholdexcept(&old_fenv)) {
    std::cerr << "Warning: could not hold floating-point exceptions!"
              << std::endl;
  }
#endif
...
void dagmcwritemcnp_(
    char* dagfile, char* lfile, int* llen,
    const char* mcnp_version_major) {  // file with cell/surface cards
  ...
  if (lfname != "lcad") {
    // Do not overwrite a lcad file if it already exists, except if it has the
    // default name "lcad"
    if (access(lfname.c_str(), R_OK) == 0) {
      std::cout << "DagMC: reading from existing lcad file " << lfname
                << std::endl;
      return;
    }
  }
  ...
  if (workflow_data->tally_library.size() > 0) write_tally_data(lcadfile_str);
...
void write_cell_cards(std::ostringstream& lcadfile,
                      const char* mcnp_version_major) {
  ...
  for (int i = 1; i <= num_cells; ++i) {
    ...
    if (workflow_data->material_library.size() == 0) {
      ...
      if (dagmc_util::to_lower(mat_num) != dagmc_util::to_lower(DMD->graveyard_mat_str()) &&
        ...
        if (!DMD->try_to_make_int(mat_num)) {
          std::cerr << "Failed to cast material number to an integer"
                    << std::endl;
          std::cerr << "volume with ID " << cellid << " has material assignment"
                    << std::endl;
          std::cerr << mat_num
                    << " which appears to be a name rather than an integer"
                    << std::endl;
      ...
      if (dagmc_util::to_lower(mat_num) != dagmc_util::to_lower(DMD->graveyard_mat_str()) &&
        ...
        pyne::Material material =
            workflow_data->material_library.get_material(mat_name);
    ...
    for (it = set.begin(); it != set.end(); ++it) {
      ...
      if (mcnp_version_major[0] == '5') {
        ...
        std::cerr << "Unknown MCNP verison: " << mcnp_version_major
                  << std::endl;
        exit(EXIT_FAILURE);
      }
      double imp = 1.0;
...
void write_material_data(std::ostringstream& lcadfile) {
  const pyne::MaterialLibrary& material_library =
      workflow_data->material_library;
...
void write_tally_data(std::ostringstream& lcadfile) {
  std::map<std::string, pyne::Tally> tally_library =
      workflow_data->tally_library;
  // loop over all tallies
  std::cout << "Writing Tallies ..." << std::endl;
  int count = 1;
  for (std::map<std::string, pyne::Tally>::iterator it = tally_library.begin();
...
void dagmcangl_(int* jsu, double* xxx, double* yyy, double* zzz, double* ang) {
  ...
  std::cout << "    : " << aa << " deg to uvw" << (aa > 90.0 ? " (!)" : "")
            << std::endl;
#endif
}

void dagmcchkcel_by_angle_(double* uuu, double* vvv, double* www, double* xxx,
                           double* yyy, double* zzz, int* jsu, int* i1,
                           int* j) {
#ifdef TRACE_DAGMC_CALLS
  std::cout << " " << std::endl;
  std::cout << "chkcel_by_angle: vol=" << DAG->id_by_index(3, *i1)
            << " surf=" << DAG->id_by_index(2, *jsu) << " xyz=" << *xxx << " "
            << *yyy << " " << *zzz << std::endl;
  std::cout << "               : uvw = " << *uuu << " " << *vvv << " " << *www
            << std::endl;
#endif
  ...
  int result;
  moab::ErrorCode rval =
      DAG->test_volume_boundary(vol, surf, xyz, uvw, result, &history);
  if (moab::MB_SUCCESS != rval) {
  ...
  if (moab::MB_SUCCESS != rval)
    *j = -2;
    ...
    switch (inside) {
      ...
      case -1:
        *j =
            1;  // onboundary== -1 -> on boundary -> j=1 (assume leaving volume)
        break;
      default:
...
void dagmcdbmin_(int* ih, double* xxx, double* yyy, double* zzz, double* huge,
  ...
  if (moab::MB_SUCCESS != rval) {
    *dbmin = *huge;
    std::cerr << "DAGMC: error in closest_to_location, returning huge value "
                 "from dbmin_"
              << std::endl;
  }

#ifdef TRACE_DAGMC_CALLS
  std::cout << "dbmin " << DAG->id_by_index(3, *ih) << " dist = " << *dbmin
            << std::endl;
#endif
}
...
void dagmcnewcel_(int* jsu, int* icl, int* iap) {
  ...
  if (moab::MB_SUCCESS != rval) {
    *iap = -1;
    std::cerr << "DAGMC: error calling next_vol, newcel_ returning -1"
              << std::endl;
  }
...
#ifdef TRACE_DAGMC_CALLS
  std::cout << "newcel: prev_vol=" << DAG->id_by_index(3, *icl)
            << " surf= " << DAG->id_by_index(2, *jsu)
            << " next_vol= " << DAG->id_by_index(3, *iap) << std::endl;
...
void dagmc_surf_reflection_(double* uuu, double* vvv, double* www,
  ...
  std::cout << "surf_reflection: " << angle(oldv, newv) * (180.0 / M_PI)
            << std::endl;
  ;
#endif
...
void dagmctrack_(int* ih, double* uuu, double* vvv, double* www, double* xxx,
  ...
  } else if (last_uvw[0] == *uuu && last_uvw[1] == *vvv &&
             last_uvw[2] == *www) {
  ...
  moab::ErrorCode result =
      DAG->ray_fire(vol, point, dir, next_surf, next_surf_dist, &history,
  ...
  // Return results: if next_surf exists, then next_surf_dist will be nearer
  // than dist_limit (if any)
  if (next_surf != 0) {
  ...
  if (raystat_dump) {
    ...
    *raystat_dump << std::accumulate(trv.nodes_visited().begin(),
                                     trv.nodes_visited().end(), 0)
                  << ",";
    *raystat_dump << std::accumulate(trv.leaves_visited().begin(),
                                     trv.leaves_visited().end(), 0)
                  << std::endl;
  }
#endif
  ...
  std::cout << "track: vol=" << DAG->id_by_index(3, *ih)
            << " prev_surf=" << DAG->id_by_index(2, *jsu)
            << " next_surf=" << DAG->id_by_index(2, *jap) << " nps=" << *nps
            << std::endl;
  std::cout << "     : xyz=" << *xxx << " " << *yyy << " " << *zzz
            << " dist = " << *dls << std::flush;
...
void dagmc_bank_push_(int* nbnk) {
  if (((unsigned)*nbnk) != history_bank.size()) {
    std::cerr << "bank push size mismatch: F" << *nbnk << " C"
              << history_bank.size() << std::endl;
  }
...
void dagmc_bank_usetop_() {
  ...
  if (history_bank.size()) {
    ...
    std::cerr << "dagmc_bank_usetop_() called without bank history!"
              << std::endl;
  }
}
...
void dagmc_bank_pop_(int* nbnk) {
  if (((unsigned)*nbnk) != history_bank.size()) {
    std::cerr << "bank pop size mismatch: F" << *nbnk << " C"
              << history_bank.size() << std::endl;
  }
...
#ifdef TRACE_DAGMC_CALLS
  std::cout << "getpar: " << *n << " (" << pblcm_history_stack[*n].size() << ")"
            << std::endl;
#endif
...
void dagmc_set_settings_(int* fort_use_dist_limit, int* use_cad,
  ...
  if (*srccell_mode) {
    std::cout
        << "DAGMC source cell optimization is ENABLED (warning: experimental!)"
        << std::endl;
  }
